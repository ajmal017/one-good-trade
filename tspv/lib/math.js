...
// define some temp vars, to minimize function calls
  ...
  gamma = nd1/(S*v*sqt);
  vega = S*sqt*nd1;
  // From source: theta = -(S*v*nd1)/(2*sqt) - r*X*ert*Nd2;
  // From ed: theta = - 0.5 * gamma * (stock price * vol)^2
  theta = -0.5*gamma*S*v*S*v;
  rho = X*t*ert*Nd2;
...
option_implied_volatility = function(call,S,X,r,t,o) { 
// call = Boolean (to calc call, call=True, put: call=false)
// S = stock prics, X = strike price, r = no-risk interest rate
// t = time to maturity
// o = option price
...
// define some temp vars, to minimize function calls
  ...
  for (i=0;i<MAX_ITER;i++) {
    data = black_scholes(call,S,X,r,sigma,t);
    price = data["price"];
    diff = o-price;
...
get_iv = function(call,s,x,r,t,o) {
  return option_implied_volatility(call,s,x,r/100,t/365,o);
}